...
<body class="sb-nav-fixed">
    ...
    try {
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        <h1 class="mt-2 my-2">Security - Out</h1>
                        <?php
                        ...
                        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['gatepass_id'])) {
                            ...
                            if ($remark === '') {
                                ...
                                if ($gpRow) {
                                    ...
                                    try{
                                        if ($gpRow->HO_STATUS == 'GATEPASS_APPROVED' && $gpRow->HO_MATERIAL_OWNER == 'BHEL-HPVP') {
                                            // 1st movement of BHEL-HPVP materials (no items input boxes)
                                            $prepstmtupd = $dBhandler->prepare("UPDATE HPVP_OSGP SET HO_TIME_IN_OUT = ?, HO_TIME_IN_OUT_BY = ?, HO_STATUS = 'SECURITY_OUT_DONE', HO_SECURITY_REMARK = ? WHERE HO_GATEPASS_SLNO = ?");
                                            $prepstmtupd->execute([$currentTime, $id, $remark, $gpslno]);
                                            if ($prepstmtupd->rowCount() > 0) {
                                                $dBhandler->commit();
                                                echo "<h4 class='text-primary'>Gatepass #" . htmlspecialchars($gpslno) . " is made OUT</h4>";
                                            }
                                        } else {
                                            // Return movement (contains item inputs)
                                            $retslno = $_POST['return_id'];
                                            $hasUpdates = false;
                                            // Update return header status to 'SECURITY_OUT_DONE'
                                            $prepstmtupdHdr = $dBhandler->prepare("UPDATE HPVP_OSGP_RETURN 
                                                SET HOR_STATUS = 'SECURITY_OUT_DONE', HOR_RETURN_DATE = ?, HOR_SECURITY_IN_OUT_BY = ?, HOR_SECURITY_REMARK = ?
                                            ...
                                            foreach ($_POST as $postName => $qtyValue) {
                                                if (str_starts_with($postName, 'qty-')) {
                                                    ...
                                                    if (is_numeric($itemslno) && $qtyValue > 0) {
                                                        // Check if a RETURN_APPROVED row exists for this gatepass and item
                                                        ...
                                                        if ($prepstmtupd->rowCount() > 0) {
                                                            echo "<h4 class='text-primary'>Gatepass #" . htmlspecialchars($gpslno) . ", Item No: " . htmlspecialchars($itemslno) . " is made OUT</h4>";
                                                            $hasUpdates = true;
                                                        }
                                                    }
                                                }
                        ...
                        $prepstmt = $dBhandler->prepare("SELECT HO_GATEPASS_SLNO, HO_REPRESENTATIVE_NAME, HO_REPRESENTATIVE_ID, HO_ORDER, HO_BHEL_OFFICIAL, HO_SOURCE_FROM, HO_DESTINATION_TO, 
                            ...
                            WHERE (HO_STATUS = 'GATEPASS_APPROVED' AND HO_MATERIAL_OWNER = 'BHEL-HPVP') 
                            OR (HOR_STATUS = 'RETURN_APPROVED' AND HO_MATERIAL_OWNER = 'Vendor')");
                        $prepstmt->execute();
                        ?>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <?php
                                                 ...
                                                 if (!($row->HO_STATUS == 'GATEPASS_APPROVED' && $row->HO_MATERIAL_OWNER == 'BHEL-HPVP')) {
                                                     $filtered_items = [];
                                                     foreach ($items as $item) {
                                                         $reqStmt = $dBhandler->prepare("SELECT HORI_ITEM_RETURN_QUANTITY_REQUEST FROM HPVP_OSGP_RETURN_ITEM, HPVP_OSGP_RETURN 
                                                         WHERE HORI_RETURN_SLNO = HOR_RETURN_SLNO AND HORI_GATEPASS_SLNO = ? AND HORI_ITEM_SLNO = ? AND HOR_STATUS = 'RETURN_APPROVED'");
                                                 ...
                                                 foreach ($items as $idx => $item) {
                                                     ...
                                                     if ($row->HO_STATUS == 'GATEPASS_APPROVED' && $row->HO_MATERIAL_OWNER == 'BHEL-HPVP') {
                                                         ...
                                                         $retStmt = $dBhandler->prepare("SELECT IFNULL(SUM(HORI_ITEM_RETURN_QUANTITY),0) RET_QTY FROM HPVP_OSGP_RETURN, HPVP_OSGP_RETURN_ITEM 
                                            ...
                                            <td>
                                                <form id="form-<?php echo $row->HO_GATEPASS_SLNO; ?>" method="post" onsubmit="return confirm('Are you sure you want to log OUT for Gatepass #<?php echo $row->HO_GATEPASS_SLNO; ?>?');"></form>
                                                ...
                                                <?php if ($row->HOR_STATUS == "RETURN_APPROVED") { 
                                                    ...
                                                    ?>                                                    
                                                    <?php } ?>        
                                                <input id="submit-<?php echo $row->HO_GATEPASS_SLNO; ?>" class="btn btn-sm btn-primary my-2" type="submit" value="OUT" form="form-<?php echo $row->HO_GATEPASS_SLNO; ?>" />
        ...
        <script src="js/security-out.js"></script>
        <?php
